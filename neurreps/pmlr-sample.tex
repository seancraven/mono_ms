...
\begin{equation}
  ...
  \label{eq:gs_mdp_rw}
\end{equation}
Here $S, S' \in \matchal{S}$ are states, and $A \in \mathcal{A}$ are actions. $g$ are group actions acting on the state action space. This definition is taken from \cite{van2020plannable}, with slightly altered notation. These notions can be extended to stocastic MDPs trivially.
